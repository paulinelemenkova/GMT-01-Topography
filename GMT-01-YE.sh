...
gmt grdcut ETOPO1_Ice_g_gmt4.grd -R42/55/10/20 -Gye_relief.nc
#gmt grdcut GEBCO_2019.nc -R38/49/29/38 -Gye_relief.nc
...
# Add isolines
gmt grdcontour ye_relief.nc -R -J -C500 -Wthinner,darkbrown -O -K >> $ps
...
-F+jTL+f10p,25,red+jLB+a-345 >> $ps << EOF
53.0 17.9 O   M   A   N
EOF
gmt pstext -R -J -N -O -K \
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,25,red+jLB+a-330 -Gwhite@30 >> $ps << EOF
42.1 11.5 DJIBOUTI
EOF
gmt pstext -R -J -N -O -K \
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,25,red+jLB+a-50 -Gwhite@30 >> $ps << EOF
42.0 13.4 ERITREA
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f8p,25,red+jLB+a-60 -Gwhite@30 >> $ps << EOF
42.0 10.9 ETHIOPIA
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,25,blue+jLB+a-335 -Gwhite@60 >> $ps << EOF
46.5 11.8 G u l f   o f   A d e n
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f11p,13,royalblue4+jLB -Gwhite@40 >> $ps << EOF
53.4 15.5 Arabian
53.6 15.2 Sea
EOF
gmt pstext -R -J -N -O -K \
...
46.1 18.4 (Empty Quarter Desert)
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,26,blue2+jLB+a-77 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,13,darkbrown+jLB+a-345 -Gwhite@70 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+f11p,13,khaki1+jLB >> $ps << EOF
50.1 16.8 Jabal Bin
50.1 16.5 Kushayt
EOF
gmt pstext -R -J -N -O -K \
-F+f11p,13,khaki1+jLB+a-45 >> $ps << EOF
44.6 17.3 Ramlat Dahm
EOF
gmt pstext -R -J -N -O -K \
-F+f11p,13,khaki1+jLB+a-330 >> $ps << EOF
46.4 15.3 Ramlat
...
# Add subtitle
gmt pstext -R0/10/0/15 -JX10/10 -X0.3c -Y8.0c -N -O \
