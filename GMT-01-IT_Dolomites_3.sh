...
gmt grdcut GEBCO_2023.nc -R4/22/35/49 -Git_reliefPoly.nc
...
#gmt makecpt -Cterra -V -T-4151/4655 > pauline.cpt
gmt makecpt -Cglobe -V -T-4151/4655 > pauline.cpt
# elevation etopo1 world elevation dem1 dem2 dem3 globe srtm turbo terra earth

#####################################################################
# create mask of vector layer from the DCW of country's polygon
gmt pscoast -R6/20/36/48 -JPoly/6.5i -Dh -M -EIT > Italy.txt
#####################################################################

ps=Topo_IT.ps
# Make background transparent image
# gmt grdimage it_relief.nc -Cpauline.cpt -R6/20/36/48 -JM6.5i -I+a15+ne0.75 -t40 -Xc -P -K > $ps
gmt grdimage it_reliefPoly.nc -Cpauline.cpt -R4/22/35/49 -JPoly/6.5i -I+a15+ne0.75 -t40 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour it_reliefPoly.nc -R -J -C1000 -A1000+f6p,26,darkbrown -Wthinnest,darkbrown -O -K >> $ps
...
gmt psclip -R4/22/35/49 -JPoly/6.5i Italy.txt -O -K >> $ps

# 2. create map within mask
# Add raster image
gmt grdimage it_reliefPoly.nc -Cpauline.cpt -R4/22/35/49 -JPoly/6.5i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour it1_reliefPoly.nc -R -J -C1000 -Wthinnest,darkbrown -O -K >> $ps
# Add coastlines, borders, rivers
gmt pscoast -R -J \
    -Ia/thinner,blue -Na -N1/thicker,tomato -W0.1p -Df -O -K >> $ps
# Add lakes
...
# Add color legend
gmt psscale -Dg1.5/37+w13.0c/0.15i+v+o0.3/0i+ml+e -R -J -Cpauline.cpt \
...
gmt psbasemap -R -J \
    ...
    -B+t"Location of the study area (South Tyrol) on the topographic map of Italy" -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    --MAP_ANNOT_OFFSET=0.1c \
    -Lx13.5c/-1.7c+c10+w400k+l"Polyconic projection. Scale (km)"+f \
    -UBL/0p/-40p -O -K >> $ps
...
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,0,black+jLB -Gpapayawhip@70 >> $ps << EOF
16.5 44.4 BOSNIA AND
16.5 44.0 HERZEGOVINA
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f7p,0,black+jLB -Gpapayawhip@70 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,0,black+jLB+a90 -GDARKCYAN@90 >> $ps << EOF
19.85 40.4 A L B A N I A
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,0,black+jLB+a90 -GDARKCYAN@90 >> $ps << EOF
20.85 43.6 S E R B I A
EOF
# phys geogr
gmt pstext -R -J -N -O -K \
