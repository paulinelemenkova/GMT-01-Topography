...
gmt grdcut ETOPO1_Ice_g_gmt4.grd -R29/42/-13/1 -Gtz_relief1.nc
...
gmt pscoast -R29/42/-12/-1  -JM6.5i -Dh -M -ETZ > tanzania.txt
#####################################################################
...
gmt grdgradient tz_relief.nc -Nt1 -A45 -Gtanzania_topo_i.nc
gmt grdimage tz_relief.nc -Itanzania_topo_i.nc -J -Cgray.cpt -O -K >> $ps
#gmt grdimage tz_relief.nc -I+a75+ne0.75 -J -Ctransp.cpt -O -K >> $ps
# Add isolines
gmt grdcontour tz_relief.nc -R -J -C500 -Wthinnest,dimgray -O -K >> $ps
# finish clipping and overlay basemap
gmt pscoast -R -J -O -K -Q >> $ps
#####################################################################
# Add isolines
gmt grdcontour tz_relief1.nc -R -J -C500 -Wthinner,darkbrown -O -K >> $ps
...
gmt psclip -JM -R tanzania.txt -O -K >> $ps
# 2. create map within mask
# Add raster image
gmt grdimage tz_relief.nc -Cmyocean.cpt -R29/42/-13/1 -JM6.5i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour tz_relief.nc -R -J -C500 -Wthinnest,navajowhite1 -O -K >> $ps
# Add coastlines, borders, rivers
gmt pscoast -R -J -P \
...
gmt psscale -Dg27.5/-13.0+w17.7c/0.15i+v+o0.3/0i+ml -R -J -Cmyocean.cpt \
    ...
    -Bg500f50a500+l"Colour scale: 'geo' [R=-5358/3447, H=0, C=HSV]" \
    -I0.2 -By+lm -O -K >> $ps
...
EOF
gmt pstext -R -J -N -O -K \
-F+f12p,13,black+jLB -Gwhite@50 >> $ps << EOF
35.6 -6.4 Dodoma
EOF
...
# countries
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,25,black+jLB+a-80 -Gwhite@50 >> $ps << EOF
33.8 -11.1 MALAWI
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,25,black+jLB -Gwhite@50 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f22p,13,white+jLB >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f14p,13,white+jLB >> $ps << EOF
40.5 -8.5 Indian
40.5 -9.5 Ocean
EOF
...
3.0 9.0 Digital elevation data: SRTM/GEBCO, 15 arc second resolution grid
EOF
