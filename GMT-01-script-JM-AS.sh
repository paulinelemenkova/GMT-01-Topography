...
gmt psbasemap -R -J \
    ...
    -Tdx13.2c/14.5c+w0.3i+f2+l+o0.15i \
    -Lx12.7c/-1.3c+c50+w700k+l"Mercator projection. Scale: km"+f \
    -UBL/-5p/-40p -O -K >> $ps

# Add legend
gmt psscale -Dg42/0+w16.5c/0.4c+v+o0.3/0i+ml -R47/77/0/31 -J -Cmyocean.cpt \
    --FONT_LABEL=7p,Helvetica,dimgray \
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,Helvetica,black+jLB+a-30 -Gwhite@60 >> $ps << EOF
58.2 6.0 C a r l s b e r g  R i d g e
EOF
...
2.8 15.5 GEBCO global terrain model, 15 arc sec resolution grid
EOF
