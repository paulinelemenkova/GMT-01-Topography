...
gmt pscoast -R2.5/6.5/49.5/51.7 -JB4.30/50.30/50/55/5i -Dh -M -EBE > Belgium.txt
#gmt pscoast -Dh -M -ELB > Malawi.txt
#####################################################################

# Make color palette
gmt makecpt -Cturbo -V -T-164/680 > pauline.cpt
# elevation etopo1 world elevation dem1 dem2 dem3 globe geo srtm turbo terra earth relief costa-rica

ps=Topo_BE_JB.ps
# Make background transparent image
gmt grdimage be_relief.nc -Cpauline.cpt -R2.5/6.5/49.5/51.7 -JB4.30/50.30/50/55/5i -I+a15+ne0.75 -t50 -Xc -P -K > $ps
...
gmt psclip -R2.5/6.5/49.5/51.7 -JB4.30/50.30/50/55/5i Belgium.txt -O -K >> $ps

# 2. create map within mask
# Add raster image
gmt grdimage be_relief.nc -Cpauline.cpt -R2.5/6.5/49.5/51.7 -JB4.30/50.30/50/55/5i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
...
gmt psscale -Dg2.5/49.25+w12.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg100f10a100+l"Colormap 'turbo': Google's Improved Rainbow Colormap for Visualization [R=-164/680, C=RGB]" \
    -I0.2 -By+lm -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Region of study: Uccle seismic station, Belgium" -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    --MAP_ANNOT_OFFSET=0.1c \
    -Lx11.0c/-2.3c+c10+w100k+l"Albers conic equal-area projection. Scale (km)"+f \
    -UBL/0p/-70p -O -K >> $ps

# Texts
gmt pstext -R -J -N -O -K \
-F+jTL+f17p,0,gray33+jLB >> $ps << EOF
4.05 50.60 B E L G I U M
EOF
gmt pstext -R -J -N -O -K \
...
gmt psbasemap -R -J -O -K -DjBL+w3.0c+stmp+o0.9/0.6c >> $ps
read x0 y0 w h < tmp
gmt pscoast --MAP_GRID_PEN_PRIMARY=thinnest,grey -R-10/30/35/63 -JM3.0c -Da -Glightgoldenrod1 -Bga -Wfaint -EBE+gred -Sdodgerblue -O -K -X$x0 -Y$y0 >> $ps
...
# Add subtitle
gmt pstext -R0/10/0/15 -JX10/10 -X1.5c -Y5.7c -N -O \
    -F+f10p,0,black+jLB >> $ps << EOF
0.0 9.0 Digital elevation data: GEBCO/SRTM, 15 arc sec resolution grid
EOF

# Convert to image file using GhostScript
gmt psconvert Topo_BE_JB.ps -A0.5c -E720 -Tj -Z
