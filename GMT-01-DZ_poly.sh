...
gmt pscoast -R-10/13/18/38 -JPoly/6.5i -Dh -M -EDZ > DZ.txt
#####################################################################

ps=Topo_DZ.ps
# Make background transparent image

gmt grdimage dz1_relief.nc -Cpauline.cpt -R-10/13/18/38 -JPoly/6.5i -I+a15+ne0.75 -t40 -Xc -P -K > $ps
...
gmt psclip -R-10/13/18/38 -JPoly/6.5i DZ.txt -O -K >> $ps

# 2. create map within mask
# Add raster image
gmt grdimage dz_relief1.nc -Cpauline.cpt -R-10/13/18/38 -JPoly/6.5i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
...
gmt psscale -Dg-10/16.0+w16.0c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
...
gmt psbasemap -R -J \
    ...
    --MAP_ANNOT_OFFSET=0.1c \
    -Lx14.5c/-2.4c+c10+w500k+l"American polyconic projection. Scale (km)"+f \
    -UBL/0p/-70p -O -K >> $ps
...
5.3 10.4 Elevation data: SRTM/GEBCO
EOF
