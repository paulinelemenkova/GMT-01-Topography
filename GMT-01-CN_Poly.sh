...
gmt pscoast -R70/137/15/55 -JPoly/6.5i -Dh -M -ECN > CN.txt
#####################################################################

ps=Topo_CN.ps
# Make background transparent image

gmt grdimage cn1_relief.nc -Cpauline.cpt -R70/137/15/55 -JPoly/6.5i -I+a15+ne0.75 -t70 -Xc -P -K > $ps
...
gmt psclip -R70/137/15/55 -JPoly/6.5i CN.txt -O -K >> $ps

# 2. create map within mask
# Add raster image
gmt grdimage cn1_relief.nc -Cpauline.cpt -R70/137/15/55 -JPoly/6.5i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour cn1_relief.nc -R -J -C2000 -Wthinnest,darkbrown -O -K >> $ps
# Add coastlines, borders, rivers, lakes
gmt pscoast -R -J -Ia/thinnest,blue -Na -N1/thick,tomato -W0.1p -Df -O -K >> $ps
