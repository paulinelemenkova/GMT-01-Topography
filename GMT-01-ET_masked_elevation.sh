...
# Make color palette
gmt makecpt -Celevation.cpt -V -T-3481/4326 > myocean.cpt
...
gmt psscale -Dg31.5/3+w13.3c/0.15i+v+o0.0/0i+ml -R -J -Cmyocean.cpt \
    ...
    -Bg500f50a500+l"Colormap: 'elevation' Washed-out colors for topography [R=-3481/4326, H=0, C=RGB]" \
    -I0.2 -By+lm -O -K >> $ps
...
read x0 y0 w h < tmp
gmt pscoast --MAP_GRID_PEN_PRIMARY=thin,grey -Rg -JG35.0/-6.0N/$w -Da -Ggold1 -A5000 -Bga -Wfaint -EET+gred -Sroyalblue -O -K -X$x0 -Y$y0 >> $ps
...
gmt psconvert Topo_ET_el.ps -A0.5c -E720 -Tj -Z
