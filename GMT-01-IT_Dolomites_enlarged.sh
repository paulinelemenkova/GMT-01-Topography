...
gmt grdcut ETOPO1_Ice_g_gmt4.grd -R6/18/44/48 -Git1_reliefB.nc
gmt grdcut GEBCO_2023.nc -R6/18/44/48 -Git_reliefB.nc
gmt grdinfo -M it_reliefB.nc
# gdalinfo -stats it_relief.nc

# Make color palette
#gmt makecpt -Cgeo -V -T-4151/4655 > pauline.cpt
# gmt makecpt -Cetopo1 -V -T-4151/4655 > pauline.cpt
#gmt makecpt -Cturbo -V -T-4151/4655 > pauline.cpt
#gmt makecpt -Cdem3 -V -T-4151/4655 > pauline.cpt
gmt makecpt -Cgeo -V -T-2013/4655 > pauline.cpt
# elevation etopo1 world elevation dem1 dem2 dem3 globe geo srtm turbo terra earth

#####################################################################
# create mask of vector layer from the DCW of country's polygon
gmt pscoast -R6/18/44/48 -JM6.5i -Dh -M -EIT > Italy.txt
#####################################################################

ps=Topo_IT.ps
# Make background transparent image
gmt grdimage it_reliefB.nc -Cpauline.cpt -R6/18/44/48 -JM6.5i -I+a15+ne0.75 -t40 -Xc -P -K > $ps
    
# Add isolines
gmt grdcontour it1_reliefB.nc -R -J -C1000 -A1000+f6p,26,darkbrown -Wthinnest,darkbrown -O -K >> $ps
...
gmt psclip -R6/18/44/48 -JM6.5i Italy.txt -O -K >> $ps

# 2. create map within mask
# Add raster image
gmt grdimage it_reliefB.nc -Cpauline.cpt -R6/18/44/48 -JM6.5i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour it1_reliefB.nc -R -J -C1000 -Wthinnest,darkbrown -O -K >> $ps
# Add coastlines, borders, rivers
gmt pscoast -R -J \
    -Ia/thinner,blue -Na -N1/thicker,tomato -W0.1p -Df -O -K >> $ps
# Add lakes
...
# Add color legend
gmt psscale -Dg4.5/44+w8.0c/0.15i+v+o0.3/0i+ml+e -R -J -Cpauline.cpt \
...
gmt psbasemap -R -J \
    ...
    -B+t"Location of the study area: Dolomite Mts., South Tyrol, Italy" -O -K >> $ps
...
-F+jTL+f12p,26,blue+jLB+a-0 >> $ps << EOF
12.5 44.5 Adriatic
12.9 44.2 Sea
EOF
gmt pstext -R -J -N -O -K \
...
gmt psconvert Topo_IT.ps -A2.5c -E720 -Tj -Z
