#!/bin/sh
# Purpose: shaded relief grid raster map from the GEBCO dataset (here: Uruguay)
...
gmt pscoast -R301.5/307.0/-35/-30 -Dh -M -EUY > uy.txt
#####################################################################

# Make color palette
# makecpt --help
gmt makecpt -Cgeo.cpt -V -T-110/455 > pauline.cpt
...
gmt psclip -R301.5/307.0/-35/-30 -JM6.0i uy.txt -O -K >> $ps
...
# Add color barlegend
gmt psscale -Dg301.5/-35.3+w15.2c/0.4c+h+o0.0/0i+ml -R -J -Cpauline.cpt \
...
gmt psbasemap -R -J \
    ...
    -B+t"Topographic map of Uruguay" -O -K >> $ps
...
read x0 y0 w h < tmp
gmt pscoast --MAP_GRID_PEN_PRIMARY=thin,grey -Rg -JG304/33S/$w -Da -Gbrown -A5000 -Bg -Wfaint -ESA+gpeachpuff -EUY+ggold1 -Sslategray2 -O -K -X$x0 -Y$y0 >> $ps
...
gmt psconvert Topography_UY.ps -A0.5c -E720 -Tj -Z
