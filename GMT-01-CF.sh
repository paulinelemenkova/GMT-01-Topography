...
gmt grdcut ETOPO1_Ice_g_gmt4.grd -R20/40/8/24.5 -Gcf1_relief.nc
gmt grdcut GEBCO_2019.nc -R20/40/8/24.5 -Gcf_relief.nc
gdalinfo -stats cf1_relief.nc
...
gmt pscoast -R20/40/8/24.5 -JM6.5i -Dh -M -ESD > CAR.txt
#####################################################################

ps=Topo_CF.ps
# Make background transparent image

gmt grdimage cf_relief.nc -Cpauline.cpt -R20/40/8/24.5 -JM6.5i -I+a15+ne0.75 -t40 -Xc -P -K > $ps
...
gmt psclip -R20/40/8/24.5 -JM6.5i CAR.txt -O -K >> $ps

# 2. create map within mask
# Add raster image
gmt grdimage cf_relief.nc -Cpauline.cpt -R20/40/8/24.5 -JM6.5i -I+a15+ne0.75 -Xc -P -O -K >> $ps
# Add isolines
gmt grdcontour cf1_relief.nc -R -J -C250 -A500+f7p,26,darkbrown -Wthinnest,darkbrown -O -K >> $ps
# Add coastlines, borders, rivers
gmt pscoast -R -J \
...
gmt psscale -Dg19.5/6.5+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Cpauline.cpt \
    ...
    -Bg500f100a500+l"Colormap: 'geo' Colors for global topography relief [R=-2756/3042, H, C=RGB]" \
    -I0.2 -By+lm -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    --MAP_ANNOT_OFFSET=0.1c \
    -Lx13.0c/-2.4c+c10+w500k+l"Mercator projection. Scale (km)"+f \
    -UBL/0p/-70p -O -K >> $ps
