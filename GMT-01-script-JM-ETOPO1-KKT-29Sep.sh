...
# Step-6. Add legend
gmt psscale -Dg134.5/40+w6.5i/0.15i+v+o0.3/0i+ml -Rkkt_relief.nc -J -Cmyocean.cpt \
    --FONT_LABEL=9p,Helvetica,black \
    --FONT_ANNOT_PRIMARY=7p,Helvetica,black \
    -Baf+l"Topographic color scale" \
    -I0.2 -By+lm -O -K >> $ps
# Step-7. Add shorelines
gmt grdcontour kkt_relief.nc -R -J -C1000 -W0.1p -O -K >> $ps
# Step-8. Add grid
gmt psbasemap -R -J \
...
-F+jTL+f16p,Times-Roman,blue+jLB -Gwhite@30>> $ps << EOF
145.0 53.0 Sea of Okhotsk
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f16p,Times-Roman,white+jLB >> $ps << EOF
160.0 46.0 Pacific Ocean
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,Times-Roman,black+jLB+a-295 -Gwhite@30 >> $ps << EOF
